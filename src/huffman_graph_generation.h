...
vector<LetterFrequency> countDistinctLetters(const vector<string>& words) {
    ...
    for (const string& word : words) { // itterate word strings from words vector
        for (char letter : word) { //itterate letters from string word
...
void storeDataInFile(const vector<LetterFrequency>& distinctLetters, const vector<string>& words) {
    ...
    string fileName;
    cout << "enter the file name based on assignment requirement: " ;
    cin >> fileName;
    ...
    if (outputFile.is_open()) {
        ...
        for (const auto& lf : distinctLetters) {
            outputFile << lf.letter << " " << lf.frequency;
            outputFile << endl;
        }
